...
- **`COMPONENTS.md`** — the library map at `packages/components/COMPONENTS.md`. Theme summary plus a for/not-for index of every component. Agents read this first; see [Working with AI agents](/docs/get-started/working-with-ai-agents) for the workflow and to preview or download the file from the docs site.
...
The agent reads this file, knows the available variants and sizes, knows when not to use the component, and can see a live rendered example. No other *website* surface is needed — the repo files are the interface.

See it live: [Button →](/docs/components/atoms/button) · [Working with AI agents →](/docs/get-started/working-with-ai-agents)
