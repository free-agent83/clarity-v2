...
Bespoke Node script, not Style Dictionary. `packages/tokens/build.mjs` is about 200 lines of plain Node, one optional dependency (`culori` for OKLCH↔hex). See [ADR-001](/docs/resources/adrs) for the rationale — the short version is that the shadcn-flat output required enough custom code against Style Dictionary's plugin API that rolling the pipeline was simpler than bending SD to fit.
...
**Figma:** tokens flow to Figma one-way via an export pipeline (see [ADR-003](/docs/resources/adrs) for why the sync is one-way).
