...
- **Missing `"use client"` directives added to 18 components** that had been missing them since the initial shadcn bulk-add: `accordion`, `alert-dialog`, `avatar`, `carousel`, `checkbox`, `command`, `drawer`, `hover-card`, `input-group`, `label`, `popover`, `radio-group`, `scroll-area`, `separator`, `sheet`, `sidebar`, `sonner`, `toggle`. Matches shadcn v4 canonical exactly for each file (cross-referenced via raw.githubusercontent fetch).
