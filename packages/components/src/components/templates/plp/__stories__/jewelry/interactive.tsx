...
import { generateJewelryItems, type JewelryFilterState } from "./api";
import {
  JewelryPlpGridItem,
  JewelryPlpListRow,
} from "./renderers";
import {
  JEWELRY_PINNED_IDS,
  JewelryDrawerBody,
  useJewelryFilterButtons,
} from "./filters";

export function buildJewelryCards(count: number): ReactNode[] {
  return generateJewelryItems(count).map((item) => (
    <JewelryPlpGridItem key={item.id} item={item} />
  ));
}

export function buildJewelryRows(count: number): ReactNode[] {
  return generateJewelryItems(count).map((item) => (
    <JewelryPlpListRow key={item.id} item={item} onClick={fn()} />
  ));
}
...
export function JewelryInteractive({
  ...
  initialFilterState?: Partial<JewelryFilterState>;
}) {
  const ctrl = useFilterController<JewelryFilterState>(initialFilterState);
  const buttons = useJewelryFilterButtons(ctrl);
  ...
  return (
    <AssemblyShell
      ...
      drawer={{
        content: <JewelryDrawerBody ctrl={ctrl} />,
